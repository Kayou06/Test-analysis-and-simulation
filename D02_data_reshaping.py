...
def pixel_to_coords(x_pixels, y_pixels, throat_x, SF=25.097):
    # SF = 25.097 [px/mm]
    x_pixels = np.arange(x_pixels)
    y_pixels = np.arange(y_pixels)

    x_coords = (x_pixels - throat_x) / SF
    y_coords = y_pixels / SF

    return x_coords, y_coords

u = np.load("VF BOS_12_11_1 (220) corrected/u_HS_alpha35_blur11_gaussian.npy")
v = np.load("VF BOS_12_11_1 (220) corrected/v_HS_alpha35_blur11_gaussian.npy")
u_reshaped, v_reshaped = reshape(u, v, ref_img_final)

throat_x = find_throat_position(ref_img_final)

y_pixels, x_pixels = u_reshaped.shape


x_coords, y_coords = pixel_to_coords(x_pixels, y_pixels, throat_x)


print(x_coords)


plt.plot(mask_point[:,0], mask_point[:,1], label="Mask points in px")
plt.legend()
plt.show()
