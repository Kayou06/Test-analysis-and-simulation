...
def circles_finder(img,blur_lvl,xmin,xmax,r_t,r_w):
    ...
    img = np.clip(img.astype(np.float32) * 15, 0, 255).astype(np.uint8)
    img_for_circles = img
    ...
    img_blur = bandpass_filter(img_blur, 19, 300) # this is a bandpass filter, it is good for edge highlighting
