...
def drho_dx(del_x):
    ...
    W = 
    n0 = 1.000293
    drho_dx = del_x*n0*(ZD+ZA-f)/(C*W*K*f*ZD)

    return drho_dx

def normalized_drho_dx(del_x, del_x_1):
    drho_dx = drho_dx(del_x)
    drho_dx_max = drho_dx(del_x_1)
    return drho_dx / drho_dx_max

midline_df['drho_dx'] = midline_df['x_displacement'].apply(drho_dx)
...
plt.plot(midline_df.iloc[:, 0], midline_df['normalized_drho_dx'])
plt.xlabel('x')
